...
function ThemePicker() {
  const { flavor, colorScheme, setFlavor, toggleDark } = useTelegram()
  return (
    <div className="flex items-center gap-1">
      <Button variant="ghost" size="icon" onClick={toggleDark} className="h-7 w-7">
        {colorScheme === 'dark' ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
      </Button>
      <DropdownMenu>
